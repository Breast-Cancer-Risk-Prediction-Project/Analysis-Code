...
data<-data %>%
    mutate(record_date=ifelse(record_date=="08/08/8000", NA, record_date),
          ...
          dxyear_primary1=ifelse(dxdate_primary1==8888| dxdate_primary1==888 | dxdate_primary1==7777 | dxdate_primary1==777, NA, dxdate_primary1),
          dxyear_primary1num=suppressWarnings(as.numeric(dxyear_primary1)), 
          dxyear_primary1=ifelse(dxyear_primary1num>record_year, dxyear_primary1num, ifelse(dxyear_primary1num==record_year, dxyear_primary1num+1, NA)),
          #Repeat for 2nd dx
          dxyear_primary2=ifelse(dxdate_primary2==8888| dxdate_primary2==888 | dxdate_primary2==7777 | dxdate_primary2==777, NA, dxdate_primary2),
        ...
        data<- data %>%
          dplyr::mutate(record_date=as.Date(record_date, format="%d/%m/%Y"),
                        ...
                        primary2dx_date=ymd(dxyear_primary2, truncated=2L),
...
asnumeric<-function(data, varlist){
  ...
  return(data)
        ...
        ################################################### 
         ...
         data<- data %>%
          mutate(
            age_outlier    =ifelse(age<18, 1,
                                   ifelse(age<666 & age>100, 1, 0)),
            ...
            #recommend changing outlier to 120 - 10 y
            ...
            hrtdur_outlier        =ifelse(hrt_dur<0, 1, 
                                          ifelse(hrt_dur>24 & hrt_dur<666, 1, 0)),
            hrtepdur_outlier      =ifelse(hrtep_dur<0, 1, 
                                          ifelse(hrtep_dur>24 & hrtep_dur<666,1, 0)),
            hrteonlydur_outlier   =ifelse(hrteonly_dur<0, 1,
                                          ifelse(hrteonly_dur>24 & hrteonly_dur<666, 1, 0)),
            pamets_outlier        =ifelse(pa_mets<=0, 1, 
                                          ifelse(pa_mets>30 & pa_mets<666, 1, 0)),
...
#Set all 666,777,888 to . for easier coding
...
write.csv(valdata, "FILEPATH/bcrpp_valdata.R")
