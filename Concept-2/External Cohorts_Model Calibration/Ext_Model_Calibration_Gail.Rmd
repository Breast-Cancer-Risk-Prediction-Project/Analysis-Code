...
```
```{r}
source("C:/Users/krist/Dropbox/BCRP Project/Code/Model Calibration/External Code/Functions_ExistingModelValidation.R", local=knitr::knit_global())
```
...
#Save absolute risks and absolute risk cats for Gail model for all if wanted
#write.csv(data_gail, "filepath/Gail_5yrisks.csv")
```
...
```{r}
AUCageadjust<-as.data.frame(AUCcalc_ageadjust(data_gail_validate_inv))
